...
for filename in filenames:
    ...
    def MSE(thetas, data):
        Sum = 0
        for i in range(len(data)):
            Sum = Sum + ((thetas[0] + thetas[1] * data[i][0]**1 + thetas[2]*data[i][0]**2 + thetas[3]*data[i][0]**3 + thetas[4]*data[i][0]**4 + thetas[5]*data[i][0]**5 + thetas[6]*data[i][0]**6 + thetas[7]*data[i][0]**7) - data[i][1])**2
        Sum = Sum/(len(data))
        print(Sum)
        return Sum

    x = Symbol('x')
    thetas = [.2, 0.1,0.00000001,0.0000001,0.00000001,0.00000001,0.000001, 0.0000001]
    alpha = 0.000001
    y1 =  thetas[0] + thetas[1] * x**1 + thetas[2]*x**2 + thetas[3]*x**3 + thetas[4]*x**4 + thetas[5]*x**5 + thetas[6]*x**6 + thetas[7]*x**7
    ...
    equation = thetas[0] + thetas[1]*x + thetas[2]*x**2 + thetas[3]*x**3 + thetas[4]*x**4 + thetas[5]*x**5 + thetas[6]*x**6 + thetas[7]*x**7
    xes = []

    def f(t, t1, t2, t3, t4, t5, t6, t7, t8):
        return t1 + t2*t + t3*t**2 + t4*t**3 + t5*t**4 + t6*t**5 + t7*t**6 + t8*t**7
    t = np.arange(-100,100,1)
    ft = f(t, thetas[0], thetas[1], thetas[2], thetas[3], thetas[4], thetas[5], thetas[6], thetas[7])
    filename2 = '7th order-' + filename 
    plt.plot(t, f(t,thetas[0], thetas[1], thetas[2], thetas[3], thetas[4], thetas[5], thetas[6], thetas[7] ), '-r', filename2)
